// LIVE: tiny USDT -> BNB swap on BSC to validate executeSwap response parsing.
...
const sizeUsd = Number(process.env.LIVE_SWAP_USD ?? 0.5);
if (!Number.isFinite(sizeUsd) || sizeUsd <= 0) throw new Error("LIVE_SWAP_USD must be a positive number");
...
try {
  ...
  console.log(`\n== executeSwap buy ${sizeUsd} USDT -> BNB (maxSlippage 150bps) ==`);
  const fill = await wallet.executeSwap(
    { asset: "BNB", side: "buy", sizeUsd, maxSlippageBps: 150 },
    now(),
  );
  console.log(fill);
